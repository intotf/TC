...
namespace TC.Validate
{
    ...
    public sealed class ValidBox
    {
        ...
        public IDictionary<string, object> AsHtmlAttribute()
        {
            ...
            {
                var rules = JsonConvert.SerializeObject(this.validRuleList);
        ...
        public IDictionary<string, object> AsHtmlAttribute(object attribute)
        {
            ...
            if (kvs == null)
            {
                kvs = new Dictionary<string, object>();
                var properties = System.ComponentModel.TypeDescriptor.GetProperties(attribute);
                for (var i = 0; i < properties.Count; i++)
                {
                    var p = properties[i];
                    kvs.Add(p.Name.Replace('_', '-'), p.GetValue(attribute));
                }
            }
